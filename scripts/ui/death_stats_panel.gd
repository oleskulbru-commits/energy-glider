...
func _populate_damage_rows(upgrade_state: RunUpgradeState, damage_stats: RunDamageStats) -> void:
	...
	var ranked := damage_stats.ranked_weapons(owned)
	var max_damage := 1
...
func _populate_upgrade_rows(upgrade_state: RunUpgradeState) -> void:
	...
	if upgrade_state == null:
		...
		return
	var entries := upgrade_state.death_upgrade_summary()
	if entries.is_empty():
		if _upgrades_section != null:
			_upgrades_section.visible = false
		return
