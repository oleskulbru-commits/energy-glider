...
func _verify_stats() -> void:
	...
	_fail_unless(AutoRocketScript.damage_for(0.0) == 28, "Base rocket should deal 28")
	_fail_unless(
		AutoRocketScript.damage_for(0.04) == 29,
	...
	_fail_unless(
		AutoRifleScript.crit_damage_for(AutoRocketScript.damage_for(0.0), true) == 56,
		"Rocket crit should double 28 to 56"
	)
